...
- **Agent self-update flow** (the global EdgeGuardian agent binary, not customer software): CI builds + Ed25519-signs + publishes each platform via `POST /api/v1/agent/binaries` → controller verifies signature and stores binary + `release.json` sidecar in MinIO under `public/agent/<os>/<arch>/` → agent polls `GET /api/v1/agent/latest-version` (when install-time `auto_update` is on) or is triggered by `edge-guardian --update` → downloads `/api/v1/agent/binary`, verifies sha256 + Ed25519 → exits 42 → watchdog swaps binary → rollback on repeated crashes. There is no controller push / per-org channel; all agents converge to one global latest.
