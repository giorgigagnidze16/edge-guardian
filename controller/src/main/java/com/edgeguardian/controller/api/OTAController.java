...
public class OTAController {
    ...
    public List<OtaArtifactDto> listArtifacts(@PathVariable Long orgId) {
        requireRole(orgId, OrgRole.owner, OrgRole.admin, OrgRole.operator, OrgRole.viewer);
    ...
    public OtaArtifactDto createArtifact(
        ...
        @RequestPart(value = "ed25519Sig", required = false) String ed25519Sig) throws IOException {

        var userId = requireRole(orgId, OrgRole.owner, OrgRole.admin, OrgRole.operator);
        ...
        return OtaArtifactDto.from(otaService.createArtifact(
            ...
            result.size(), result.sha256(), ed25519Sig, result.storagePath(), userId));
    }

    @DeleteMapping("/artifacts/{artifactId}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void deleteArtifact(@PathVariable Long orgId, @PathVariable Long artifactId) throws IOException {
        requireRole(orgId, OrgRole.owner, OrgRole.admin);
    ...
    private Long requireRole(Long orgId, OrgRole... roles) {
        Long userId = TenantContext.getUserId();
        if (userId == null) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "User not authenticated");
        }
        organizationService.requireRole(orgId, userId, roles);
        return userId;
    }
}