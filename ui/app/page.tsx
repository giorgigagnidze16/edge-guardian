...
export default function LoginPage() {
  ...
  const discovery = useInView(0.15);

  const doSignIn = useCallback(() => signIn("keycloak", { callbackUrl: "/dashboard" }), []);
  ...
  return (
    <div ref={containerRef} className="relative min-h-screen bg-background text-foreground [overflow-x:clip]">
      ...
      <style>{`
        ...
        .discovery-ring {
          animation: discoveryRing 3s ease-out infinite;
        }
...
/* ─── Device auto-discovery network animation ─── */

function DeviceDiscoveryMap({ active }: { active: boolean }) {
